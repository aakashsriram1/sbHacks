...
export function DailyCheckInModal({
  ...
  const handleSubmit = () => {
    // We'll gather the answers depending on the task
    if (isLeetCode) {
      onSubmit({
      ...
      // If there's another task, or you want a default, handle that here
      onSubmit({});
    }
  };
      ...
      <Card className="relative w-full max-w-md">
        ...
        <CardContent className="space-y-4">
          {isLeetCode && (
            <>
              <label className="block">
                <span className="text-sm font-medium">1) Name of the problem</span>
                <input
              ...
              <label className="block">
                <span className="text-sm font-medium">2) Describe your algorithm</span>
                <textarea
                  className="block w-full mt-1 p-2 border border-gray-300 rounded"
                  rows={3}
          ...
          {isGym && (
            <>
              <label className="block">
                <span className="text-sm font-medium">1) What workout did you do?</span>
                <input
                  className="block w-full mt-1 p-2 border border-gray-300 rounded"
                  value={gymWorkout}
              ...
              <label className="block">
                <span className="text-sm font-medium">2) How long were you there?</span>
                <input
                  className="block w-full mt-1 p-2 border border-gray-300 rounded"
                  value={gymDuration}
          ...
          {/* Feel free to add more tasks or a default set of questions if needed */}
        </CardContent>
